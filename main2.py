...
def discover_instances(root: str) -> list[dict]:
    ...
    for density_dir in sorted(os.listdir(root)):
        ...
        for size_dir in sorted(os.listdir(density_path)):
            ...
            match = re.match(r"m(\d+)_n(\d+)", size_dir)
            m_hint = int(match.group(1)) if match else None
            n_hint = int(match.group(2)) if match else None
...
def main():
    ...
    # Limpa CSVs antigos para não acumular linhas duplicadas
